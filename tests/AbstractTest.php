...
abstract class AbstractTest extends WebTestCase
{
    ...
    {
        /** @var UserPasswordHasherInterface $hash */
        $hash = static::$container->get('security.user_password_hasher');
        $loader = new Loader();

        foreach ($fixtures as $fixture) {
            if (!\is_object($fixture)) {
                $fixture = new $fixture($hash);
            }
    ...
    public function assertError(array $expectError = [], array $actualError = [])
    {
        foreach ($expectError as $key => $error)
        {
            $this->assertSame($error, $actualError[$key]);
        }
    }
